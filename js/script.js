function drawText() {
  ...
  // let colorContorno = window.prompt("Qué color quieres el contorno (en ingleis)");
  // colorContorno.toLowerCase;
  // let colorRelleno = window.prompt("Y el relleno? (en ingleis aussi)");
  // colorRelleno.toLowerCase;
  ...
  //clear context to redraw

  //wordCount bigger than 3, this is to controll the blank spaces between words

  context.clearRect(0,0,screenWidth,screenHeight);

  if(wordCount>=3){

    //to keep words legible we have to control the spaces between them, if space is less than 20px we will
    //decrease the count by one
    // if(individualBlankWidth<20) {
    //   wordCount--;
    // } 

    //block of code to print words on canvas
